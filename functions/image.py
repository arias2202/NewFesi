...
# ------------------------------------------- FUNCTIONS -----------------------------------------------
    ...
    def draw_rf_on_images(self, img_name,crop_pos, K, P, S,mode='constant'):
        ...
        """
        img = self._load_image(img_name)[0]
        img2 = self._load_image(img_name)[0]
        # Add padding
        img = img.detach().cpu().squeeze(0).permute(1, 2, 0).numpy()
        img2 = img2.detach().cpu().squeeze(0).permute(1, 2, 0).numpy()
        ...
        print(crop_pos[0]*S)
        print(crop_pos[0]*S+K+2)
        print(crop_pos[1] * S)
        print(crop_pos[1] * S + K + 2)
    ...
    def get_patch(self, img_name, crop_pos, K, P, S, mode='constant'):
        ...
        img = torch.squeeze(self._load_image(img_name,self.preprocessing_function)[0].cpu()).permute(1, 2, 0)

        # Add padding

        if P > 0:
            img = np.pad(img, ((P, P+1), (P, P+1), (0, 0)), mode)
    ...
    def destroy_all_image_but_patch(self, img_name, crop_pos, K, P, S,shuffle=False ,mode='constant'):
        """Returns a region patch from an image.
        ...
        if shuffle:
            shuffle_im=img2[x1:x2,y1:y2,:]
            np.random.shuffle(shuffle_im)
            img2[x1:x2,y1:y2,:]=shuffle_im
        return img2


    def destroy_all_image_but_patch_all_permut(self, img_name, crop_pos, K, P, S,shuffle=True ,mode='constant'):
        """Returns a region patch from an image.
        ...
        if shuffle:
            ...
            xpermutations=list(itertools.permutations(range(shufle2.shape[0])))
            ypermutations = list(itertools.permutations(range(shufle2.shape[1])))

            for Xpermutation in xpermutations:
                for Ypermutation in ypermutations:
                    for J,j in enumerate(Xpermutation):
                        for I, i in enumerate(Ypermutation):


                            shufle2[J,I]=shuffle_im[j,i]


                    img2[x1:x2,y1:y2,:]=shufle2
    ...
    def _load_image(self, img_name, preprocessing_function= None , prep_function=True):
