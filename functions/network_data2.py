...
from functions.read_activations import fill_all_layers_data_batch,get_activations_ablation_from_pos
...
class NetworkData(object):
    ...
    def layers_data(self, layer_data):
        ...
        if type(layer_data) is list:
            for layer_name in layer_data:
                ...
                layer_d.get_receptive_field(self.model,self.input_shape,main_input=0, dataset=self.dataset)
    ...
    def eval_network(self, layer_names = None,
        ...
        for n_batches, imgs in enumerate(data_batch.iterator):
            ...
            self.imatge1= [images[0],actual_file_names[0]]
            if verbose:
    ...
    def similarity_idx(self, layer_name):
        ...
        for l in layer_name:
            ...
            if layer is None:
                ...
                sim_idx.append(layer.get_similarity_idx(self, self.dataset))
            if self.save_changes:
                end_time = time.time()
                if end_time-start_time>=MIN_PROCESS_TIME_TO_OVERWRITE:
                    #Update only the modelName.obj
