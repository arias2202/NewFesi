...
def get_row_of_similarity_index(neuron, model, layer_name, dataset):
    """Returns the similarity index value. Calculates the similarity value
    ...
    if not np.isclose(max(neuron.activations), 0.0):
        ...
        new_act = read_activations.get_activation_from_pos(images=images, nefesimodel=model,
                                                           layer_name=layer_name, idx_neuron=None, pos=locations)
        # new_act=[]
        # locations = neuron.xy_locations
        # for i in range(5):
        #     images = dataset.load_images(neuron.images_id[i*20:i*20+1])
        #
        #
        #     new_act.append(read_activations.get_activation_from_pos(images=images, model=model,
        #                                                    layer_name=layer_name, idx_neuron=None, pos=locations[i*20:i+1*20]))
        # new_act=[item for sublist in new_act for item in sublist]
        new_act_sum=np.sum(new_act,axis=0)



        return new_act_sum/np.sum(neuron.activations)
    else:
        return 0.0