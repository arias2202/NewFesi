...
def zero_neuron_output_hook(index_to_zero):
    def hook(module, input, output):
        output = output.clone()  # avoid modifying in-place
        output[:, index_to_zero,:,:] = 0
        return output
    return hook
...
def get_activations_ablation_from_pos(images,nefesimodel, layer_name, lower_layer, idx_neuron, pos, batch_size = ACTIVATIONS_BATCH_SIZE):
    """Returns the activations of a neuron, given a location ('pos')
         on the activation map for each input (`images`).

        :param images: List of inputs, the inputs expected by the network.
        :param nefesimodel:The nefesi model instance.
        :param layer_name: String, name of the layer from which get the outputs.
    ...
    num_neurons=nefesimodel.get_len_neurons_of_layer(lower_layer)
    ablation_idx=np.zeros(shape=num_neurons, dtype=np.float)

    for i in range(1, len(batches)):

        total_activations = get_one_neuron_activations(nefesimodel.model, images[batches[i - 1]:batches[i]],
                                                       idx_neuron=idx_neuron, layer_name=layer_name)
        if len(total_activations.shape) > 2:
            activations = [activ[pos[t][0]][pos[t][1]] for t, activ in enumerate(total_activations)]



    original_actications= np.sum(activations)
    ...
    for n in range(num_neurons):
        activations = np.zeros(shape=batch_size, dtype=np.float)
        # Here we set to zero the neuron of the lower layer
        layer=rgetattr(model.pytorchmodel,lower_layer)
        hook_handle =layer.register_forward_hook(zero_neuron_output_hook(index_to_zero=n))
        for i in range(1, len(batches)):

            total_activations = get_one_neuron_activations(model, images[batches[i - 1]:batches[i]],
                                                           idx_neuron=idx_neuron, layer_name=layer_name)
            if len(total_activations.shape) > 2:
                activations = [activ[pos[t][0]][pos[t][1]] for t, activ in enumerate(total_activations)]
        # for each input in 'images' (range(len(activations))), get the activation value in 'pos'
        new_actications = np.sum(activations)
        if original_actications == 0:
            ablation_idx[n]=0
        else:
            ablation_idx[n] = 1- (new_actications/original_actications)
        hook_handle.remove()
...
def get_activation_from_pos(images, nefesimodel, layer_name, idx_neuron, pos, batch_size = ACTIVATIONS_BATCH_SIZE):
    """Returns the activations of a neuron, given a location ('pos')
     on the activation map for each input (`images`).
    ...
    if idx_neuron is None:
        neurons_of_layer=nefesimodel.get_len_neurons_of_layer(layer_name)
        model=nefesimodel.model
        ...
        activations = np.zeros(shape=batch_size, dtype=np.float)
        for i in range(1,len(batches)):
            total_activations = get_one_neuron_activations(nefesimodel.model, images[batches[i-1]:batches[i]],idx_neuron=idx_neuron, layer_name=layer_name)
            if len(total_activations.shape) > 2:
                activations= [activ[pos[t][0]][pos[t][1]]  for t,activ in enumerate(total_activations)]
    # for each input in 'images' (range(len(activations))), get the activation value in 'pos'
    return activations
    ...
    for activation in activations:
        if not complex_type:
            if len(activation.shape) > 0:
                activations_upsampled = np.array(PIL.Image.fromarray((activation * 255).astype(np.uint8)).resize(tuple(sz_img), PIL.Image.BILINEAR))
            else:
