...
def get_concept_selectivity_of_neuron(network_data, layer_name, neuron_idx, type='mean', concept='object', th = 0.1,
    ...
    receptive_field = [layer_data.receptive_field_Kernel,layer_data.receptive_field_Padding,layer_data.receptive_field_Stride]
...
def get_general_hist(segmentation, receptive_field, xy_locations, activations_masks, norm_activations, type, concept, target_size,
    ...
    for i, segment in enumerate(segmentation):
        ...
        K,P,S=receptive_field

        ri = xy_locations[i, 0]*S
        rf = xy_locations[i, 0]*S+K
        ci = xy_locations[i, 0] * S
        cf = xy_locations[i, 0] * S + K

        # Resize segmentation if necessary
        if target_size != object_segment.shape:
            object_segment =  Image.fromarray(object_segment).resize(target_size, Image.NEAREST)
            object_segment = np.asarray(add_padding(object_segment, P,0))

            if concept == 'part':
                # Resize all part
                part_segment = np.array(list((map(lambda part: np.array(add_padding(Image.fromarray(part).
                                                                        resize(target_size,
                                                                               Image.NEAREST),P)), list(part_segment)))))
...
def get_class_selectivity_idx_stroop(neuron_data, labels = None, threshold=.1, type=2, norm_act = None, original_norm_act = None):
    """Returns the class selectivity index value.
    :param neuron_data: The `nefesi.neuron_data.NeuronData` instance.
    :param labels: Dictionary, key: name class, value: label class.
    :param threshold: Float.
    ...
    """
    num_max_activations = len(neuron_data.activations)
    rel_freq = relative_freq_class_stroop(neuron_data, labels, norm_act=norm_act, original_norm_activations=original_norm_act)
...
def relative_freq_class_stroop(neuron_data, labels = None, norm_act = None, original_norm_activations = None):
    ...
    color_labels=neuron_data.images_id
    color_leter=neuron_data.top_labels
    color_labels = [color.split('/')[1] for color in color_labels]
    color_labels=[color.replace('/','').replace('/','').replace('.jpg','')  for c,color in enumerate(color_labels)]
    color_labels=[re.sub(pattern,'',color)[:-1] for color in  color_labels ]
    ...
    classes, classes_idx, classes_counts =  np.unique(color_labels, return_inverse=True, return_counts=True)
