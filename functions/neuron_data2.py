...
class NeuronData(object):
    ...
    """
    "/data/134-1/datasets/ImageNetFused/n01440764/ILSVRC2012_val_00000293.JPEG"
    ...
    def get_relevance_idx(self, nefesimodel ,lower_layer='.*', uper_layer=None):


        image_names = self.images_id
        locations = self.xy_locations
        images_color = nefesimodel.dataset.load_images(image_names)
        layer_data= nefesimodel.get_layer_by_name(uper_layer)
        idx_neuron = layer_data.neurons_data.index(self)
        relevance = get_activations_ablation_from_pos(images_color, nefesimodel, uper_layer, lower_layer, idx_neuron,locations)

        self.relevance_idx[lower_layer]=relevance

        return self.relevance_idx[lower_layer]
    ...
    def shape_selectivity_idx_first_layer(self, model, layer_data, dataset):
        """Returns the color selectivity index for this neuron.
        ...
        key = 'shape'
        if key not in self.selectivity_idx:
            self.selectivity_idx[key] = get_shape_selectivity_index_first_layer(self, model,
                                                     layer_data, dataset)
        return self.selectivity_idx[key]
    ...
    def max_concept_selectivity_idx(self):
        ...
        for key in self.selectivity_idx.keys():
            if key != 'color':
                suma = sum([x[1] for x in self.selectivity_idx[key] if x[1] > 0.1])
            else:
                suma=self.selectivity_idx[key]

            if suma > max:
                max = suma
                top_concept = key

        self.top_index=[top_concept,max]
    ...
    def concept_selectivity_idx(self,layer_data,network_data,neuron_idx,type='mean',concept='object',th=0.1,activations_masks=None,return_non_normalized_sum=False):

        key ='concept'+concept+str(th)
        if key not in self.selectivity_idx or (return_non_normalized_sum and (key not in self.selectivity_idx_non_normalized_sum)):
            if isinstance(layer_data,str):
                layer_data=network_data.get_layer_by_name(layer_data)
            self.selectivity_idx[key],self.selectivity_idx_non_normalized_sum[key]=get_concept_selectivity_of_neuron(network_data=network_data,layer_name=layer_data.layer_id,neuron_idx=neuron_idx,type=type,concept=concept,th=0.1,activations_masks=activations_masks,return_non_normalized_sum=True)
    ...
    def class_selectivity_color_idx(self, labels=None, threshold=.1):
        """Returns the class selectivity index for this neuron.
        :param labels: Dictionary, key: name class, value: label class.
            This argument is needed for calculate the class index.
        :param threshold: Float, required for calculate the class index.
        ...
        :raise:
            TypeError: If `labels` is None or not a dictionary.
        """
        key = 'class_color' + str(threshold)
        if key not in self.selectivity_idx:
            # Labels always must to be a dictionary
            if type(labels) is not dict and labels is not None:
                raise TypeError("The 'labels' argument should be a dictionary if is specified")
            self.selectivity_idx[key] = get_class_selectivity_idx_stroop(self, labels, threshold)
