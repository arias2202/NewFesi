...
def plot_similarity_tsne(layer_data, n=None):
    ...
    x = 1-(layer_data.get_similarity_idx(neurons_idx=idx_neurons))
    x= np.asarray([[n.selectivity_idx['shape'],n.selectivity_idx['color'],np.sum([nn[1] for nn in n.selectivity_idx['conceptobject0.1']]),np.sum([nn[1] for nn in n.selectivity_idx['class0.1']])] for n in neurons])
